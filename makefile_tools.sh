...
# Run the spinner animation logic
_run_spinner_logic() {
  ...
  while true; do
    for char in "${chars_to_use[@]}"; do
      ...
      printf "%b%s%b" "$SPINNER_COLOR" "$char" "$COLOR_RESET" # Display the spinner
...
start_spinner() {
  ...
  if [[ -n "$custom_color" ]]; then
    SPINNER_COLOR="$(_get_color_code "$custom_color")"
  fi
...
_export_spinner_functions() {
  ...
  export -f start_spinner stop_spinner _run_spinner_logic _is_supported_utf8 
}
